function SSD = findSSD_separatedensity(params, phi_2_tilde, phi_3_tilde)

if length(params)~=12
    error('There should be 12 parameters in the first input argument')
end
...
nu_2_tilde = params(9);
alpha_2_tilde = params(10);
nu_3_tilde = params(11);
alpha_3_tilde = params(12);


% equilibrium of the assumed model: 
a = sigma_1*gamma/(1-sigma_2*(1-delta));
b = sigma_2*delta/(1-sigma_3);
nhat1_eq_expected = alpha_3_hat/(a*b)*(phi_3_hat*nu_3_hat*a*b/(1-sigma_1*(1-gamma))-1);
nhat_eq_expected = [nhat1_eq_expected, a*nhat1_eq_expected, a*b*nhat1_eq_expected];
        ...
        for i = 2:tmax
            n1_tilde(i) = phi_2_tilde*nu_2_tilde/(1+n2_tilde(i-1)/alpha_2_tilde)*n2_tilde(i-1)...
                        + phi_3_tilde*nu_3_tilde/(1+n3_tilde(i-1)/alpha_3_tilde)*n3_tilde(i-1) ...
                        + sigma_1*(1-gamma)*n1_tilde(i-1);
